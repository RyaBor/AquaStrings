...
void setup() {
    Serial.begin(115200);
    Serial.println("\n--- Laser Harp System Initializing (Low Latency + Smart Mute) ---");
...
// =================================================================
/**
 * @brief Main audio mixer task with smart amplifier muting to eliminate idle noise.
 */
void audioMixerTask(void *pvParameters) {
    uint16_t i2s_output_buffer[MIXER_BUFFER_SAMPLES * 2];

    // State variable to track if the amplifier is muted.
    bool systemIsMuted = true;
    // Mute the amplifier on startup.
    audioamp.enableChannel(false, false); 

    while (true) {
        // --- Smart Muting Logic ---
        // First, check if any notes are currently supposed to be playing.
        bool isAnyNotePlaying = false;
        for (int i = 0; i < NUM_SENSORS; i++) {
            if (notes[i].isPlaying) {
                isAnyNotePlaying = true;
                break; // Found one, no need to check the rest.
            }
        }

        // If notes should be playing but the system is muted, wake up the amplifier.
        if (isAnyNotePlaying && systemIsMuted) {
            audioamp.enableChannel(true, true); // Un-mute amplifier
            systemIsMuted = false;
            delay(1); 
        } 
        // If no notes are playing but the system is not muted, shut down the amplifier.
        else if (!isAnyNotePlaying && !systemIsMuted) {
            audioamp.enableChannel(false, false); // Mute amplifier
            systemIsMuted = true;
        }
